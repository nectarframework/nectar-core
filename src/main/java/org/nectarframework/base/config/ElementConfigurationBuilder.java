...
public class ElementConfigurationBuilder implements ConfigurationBuilder<BasicConfig> {

	@Override
	public Configuration buildConfiguration(BasicConfig basicConfig) throws ConfigurationException {
		try {
			Element configElm = ElementReader.read(basicConfig.getConfigFile());
			return new Configuration(parseFullConfig(configElm), basicConfig);
		} catch (InstantiationException | IllegalAccessException | IOException e) {
			throw new ConfigurationException(e);
		}
	}

	private HashMap<String, List<Service>> parseFullConfig(Element configElm) throws ConfigurationException {
		// open config.xml file, parse each service and parameters.
		HashMap<String, List<Service>> serviceListByNodeGroup = new HashMap<String, List<Service>>();
		...
		for (Element node : configElm.getChildren("node")) {
			...
			for (Element serviceElm : node.getChildren("service")) {
				...
				ServiceParameters serviceParams = ServiceParameters.parseServiceParamtersFromElement(serviceElm);

				service.setParameters(serviceParams);

				serviceList.add(service);
			}
			serviceListByNodeGroup.put(nodeGroupName, serviceList);
		}

		return serviceListByNodeGroup;
	}

	private static Element expandInheritance(Element configElm) throws ConfigurationException {
